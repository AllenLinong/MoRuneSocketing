...
- **版本**：1.0.6-Release
- **API 版本**：Paper 1.21.1-R0.1-SNAPSHOT
- **Java 要求**：JDK 21+
- **作者**：Allen\_Linong
- **联系方式**：QQ 1422163791
...
- 成功率系统：可配置的镶嵌成功率
- 多核心兼容：内置 FoliaLib，支持 Paper/Folia/Spigot
- 第三方附魔支持：支持自定义命名空间的附魔插件
- 多语言支持：支持中文和英文
...
调度器兼容工具类，封装 FoliaLib，统一 Paper/Folia/Spigot 的调度操作。
...
// 延迟执行
public static WrappedTask runTask(Runnable task)
public static WrappedTask runTaskLater(Runnable task, long delayTicks)
...
public static WrappedTask runTaskTimerAsync(Runnable task, long delayTicks, long periodTicks)
```
...
符文ID:
  ...
  material: "物品材质"
  # 附魔类型（非buff符文使用）
  # 支持原版附魔（默认 minecraft: 命名空间，如 sharpness）
  # 支持第三方附魔（需要完整命名空间，如 fotia:water_mastery）
  enchantment: "附魔名称"
  enchant-level: 1
  # Buff类型（buff符文使用）
  buff: "buff标识"
...
- **1.0.6-Release**：升级 Paper API 1.21.1，集成 FoliaLib 支持 Folia 核心，新增 `/mrs analyze` 命令，支持第三方附魔命名空间
